...
def train_epoch(
    ...
    for cur_iter, (inputs, kpt_attn_masks, labels, _, meta) in enumerate(train_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.MODEL.MODEL_NAME == 'vit_poseblock_auxloss_patch16_224':
                preds, poseblock_attentions, gt_mask = model(inputs, kpt_attn_masks)
            else:
                preds = model(inputs, kpt_attn_masks)

        # Compute the loss.
        if cfg.MODEL.MODEL_NAME == 'vit_poseblock_auxloss_patch16_224':
            loss = loss_fun(preds, poseblock_attentions, labels, gt_mask)
        else:
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None, wandb_run=None):
    ...
    for cur_iter, (inputs, kpt_attn_masks, labels, _, meta) in enumerate(val_loader):
        if cfg.NUM_GPUS:
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.DATA.MULTI_LABEL:
                ...
                if wandb_run is not None:
                    class_names = [f'c{x}' for x in range(31)] # DEBUG TESTING

                    wandb_run.log(
                        {
                            # "val_conf_matrix": wandb.plot.confusion_matrix(
                            #         probs=None, 
