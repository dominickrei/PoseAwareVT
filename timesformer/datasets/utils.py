...
def spatial_sampling(
    ...
    Returns:
        ...
        keypoints (np.ndarray or None): updated keypoints corresponding to spatially sampled frames
    """
    assert spatial_idx in [-1, 0, 1, 2]
    if spatial_idx == -1:
        frames, _, keypoints = transform.random_short_side_scale_jitter(
            images=frames,
            min_size=min_scale,
            max_size=max_scale,
        ...
        frames, _, keypoints = transform.random_crop(frames, crop_size, keypoints=keypoints)
        if random_horizontal_flip:
            frames, _, keypoints = transform.horizontal_flip(0.5, frames, keypoints=keypoints)
    else:
        # The testing is deterministic and no jitter should be performed.
        # min_scale, max_scale, and crop_size are expect to be the same.
        #assert len({min_scale, max_scale, crop_size}) == 1
        frames, _, keypoints = transform.random_short_side_scale_jitter(
            frames, min_scale, max_scale, keypoints=keypoints
        )
        frames, _, keypoints = transform.uniform_crop(frames, crop_size, spatial_idx, keypoints=keypoints)
