...
def temporal_sampling(frames, start_idx, end_idx, num_samples):
    ...
    if frames.shape[0] == 1:
        upper_lim = frames.shape[0] - 1
    else:
        upper_lim = frames.shape[0] - 2
        
    index = torch.clamp(index, 0, upper_lim).long() # - 2 because some frames do not have keypoints for last frame
...
, duration=None, frames_length=None):
    ...
    # print(f'Number of video frames: {frames_length}')
    duration = container.streams.video[0].duration
    # print(f'Duration of video: {duration}')
    ...
    if duration is None:
        ...
        clip_start_frame, clip_end_frame = 0, frames_length
    else:
        ...
        # print(f'Start idx: {start_idx} \t End idx: {end_idx}') # debug
        timebase = duration / frames_length
        video_start_pts = int(start_idx * timebase)
        video_end_pts = int(end_idx * timebase)
        clip_start_frame, clip_end_frame = int(start_idx), int(end_idx)
    ...
    return frames, fps, decode_all_video, clip_start_frame, clip_end_frame
...
def decode(
    ...
    Returns:
        frames (tensor): decoded frames from the video.
        sampled_frames (List[int]): the indices of the sampled frames (relative to the entire video)
    """
    # Currently support two decoders: 1) PyAV, and 2) TorchVision.
    assert clip_idx >= -1, "Not valied clip_idx {}".format(clip_idx)
    try:
        if backend == "pyav":
            frames, fps, decode_all_video, clip_start_frame, clip_end_frame = pyav_decode(
                container,
                sampling_rate,
                num_frames,
                clip_idx,
            ...
            raise NotImplementedError("Dual branch transformer does not work with torchvision backend. Need to implement clip -> keypoint idxs")
    ...
    sampled_frames = index + clip_start_frame
    # print(f'Sampled frame idxs: {sampled_frames}')
