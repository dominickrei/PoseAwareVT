...
# For Base-TimeSformer & Human-Timesformer: if True will initialize model with weights at TIMESFORMER.PRETRAINED_MODEL or DUAL_BRANCH_TIMESFORMER.PRETRAINED_MODEL if exists.
#                                           Otherwise weights will be initialized with ViT weights. i.e., set this to False to train from scratch
# TRAIN.FINETUNE should be set to false for this cfg option to matter
_C.TRAIN.PRETRAINED = True

# Whether to finetune the model on given checkpoint
# For Base-Timesformer & Human-Timesformer: if True will initialize the model with weights at TRAIN.CHECKPOINT_FILE_PATH
# For Human-Timesformer:
#           Only use this for loading training checkpoints from Human-Timesformer model
#           For loading Kinetics weights for pretraining, see DUAL_BRANCH_TIMESFORMER.PRETRAINED_MODEL
...
##
##
####### Dual Branch TimeSformer Options
##
##
_C.DUAL_BRANCH_TIMESFORMER = CfgNode()
_C.DUAL_BRANCH_TIMESFORMER.ATTENTION_TYPE = 'divided_space_time'

# Path to pretrained Kinetics model to initialize from. TRAIN.PRETRAINED should be True for this to load
# Model loading using this config option is very sensitive. Only use path to 'TimeSformer_divST_8x32_224_K400.pyth'
# For resuming training from checkpoint, see TRAIN.FINETUNE and TRAIN.CHECKPOINT_FILE_PATH
_C.DUAL_BRANCH_TIMESFORMER.PRETRAINED_MODEL = ''

_C.DUAL_BRANCH_TIMESFORMER.DEPTH = 1 # Number of dual-branch-timesformer blocks
_C.DUAL_BRANCH_TIMESFORMER.HUMAN_DEPTH = 1 # Number of human AttnBlocks per dual-branch-timesformer block
_C.DUAL_BRANCH_TIMESFORMER.BG_DEPTH = 1 # Number of bg AttnBlocks per dual-branch-timesformer block

_C.DUAL_BRANCH_TIMESFORMER.MERGE_INPUT = 'all' # 'all' or 'cls_token'
_C.DUAL_BRANCH_TIMESFORMER.MERGE_FUSION_TYPE = 'concat' # average or concat
_C.DUAL_BRANCH_TIMESFORMER.POSE_INFLATION = 1 # How far to "inflate" joint-containing patches into neighboring patches. None to disable
...
# Added by Dominick: If True, perform data augmentation on train/test
# If False, do not perform data augmentation on train/test
...
# -----------------------------------------------------------------------------
# Weights and Biases Options - You should login to wandb beforehand
# -----------------------------------------------------------------------------
_C.WANDB = CfgNode()

# Log stats to weights and biases
_C.WANDB.ENABLE = False

# Disable printing from wandb
_C.WANDB.SILENT = False

_C.WANDB.PROJECT_NAME = 'timesformer'

_C.WANDB.EXPERIMENT_NAME = ''

_C.WANDB.RUN_ID = ''

_C.WANDB.SAVE_LAST_CHECKPOINT = True

_C.WANDB.SAVE_OPTIMIZER_STATE = False
